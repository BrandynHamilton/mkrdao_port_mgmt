...
rl_simulator = RL_VaultSimulator(simulation_data, test_data_copy, features, targets, temporals, start_date, end_date, scale_factor=300000000, minimum_value_factor=0.05, volatility_window=250)
...
#rl_simulator.plot_simulation_results()
rl_action_df = rl_environment.generate_action_dataframe()

mvo_simulator = RL_VaultSimulator(simulation_data, test_data_copy, features, targets, temporals, start_date, end_date, scale_factor=300000000, minimum_value_factor=0.05, volatility_window=250)
...
#mvo_simulator.plot_simulation_results()
...
rl_sim_dai_ceilings = rl_simulator.dai_ceilings_history
mvo_sim_dai_ceilings = mvo_simulator.dai_ceilings_history
...
rl_action_df['dai ceilings'].dropna().to_dict()


# In[893]:


rl_action_df['target weights'].dropna().to_dict()


# In[894]:


rl_action_df['current composition'].dropna().to_dict()


# In[895]:


rl_action_df['current sortino ratio'].dropna().plot()


# In[896]:


#rl_action_df['current cumulative return'].dropna().plot()


# In[897]:


rl_action_df['action'].dropna().to_dict()
...
#action_df
rl_actions_log_path = 'results/rlactionslog.csv'
rl_action_df.to_csv(rl_actions_log_path)

mvo_actions_log_path = 'results/mvoactionslog.csv'
mvo_action_df.to_csv(mvo_actions_log_path)


# In[899]:


rl_simulations_path = 'results/rlsimulations.csv'
rl_simulator.results.to_csv(rl_simulations_path)

mvo_simulations_path = 'results/mvosimulations.csv'
mvo_simulator.results.to_csv(mvo_simulations_path)
...
rl_combined_ceiling_data = pd.concat([historical_ceilings_for_sim, rl_sim_dai_ceilings])
mvo_combined_ceiling_data = pd.concat([historical_ceilings_for_sim, mvo_sim_dai_ceilings])
...
plt.show() 
print(f'rl current portfolio value: ${rl_total_portfolio_value.iloc[-1]:,.2f}')


plt.pie(mvo_portfolio_composition.iloc[-1], labels = mvo_portfolio_composition.columns, autopct='%1.1f%%')
plt.show() 
print(f'mvo current portfolio value: ${mvo_total_portfolio_value.iloc[-1]:,.2f}')
...
plt.show() 
print(f'historical current portfolio value: ${historical_total_portfolio_value.iloc[-1]:,.2f}')
